...
export default function PathReveal(props: any) {
    ...
    const [computedStrokeWidth, setComputedStrokeWidth] = React.useState<number>(beamWidth) // Stroke width adjusted for zoom
    ...
    const findStickyParent = React.useCallback((el: HTMLElement): HTMLElement | null => {
    ...
    const getOriginalPosition = React.useCallback((element: HTMLElement): number => {
    ...
    useEffect(() => {
        ...
        const updateProgress = () => {
            ...
            const linePx = startAlign === "center" ? vh / 2 : startAlign === "bottom" ? vh : 0
    ...
    useLayoutEffect(() => {
        ...
        if (bbox && bbox.width > 0 && bbox.height > 0) {
            ...
            const strokePadding = Math.max(0, (computedStrokeWidth || beamWidth) / 2)
            setViewBox(
    ...
    useEffect(() => {
        const compute = () => {
            ...
            setComputedStrokeWidth((prev) => (Math.abs(prev - adjustedStrokeWidth) > 0.05 ? adjustedStrokeWidth : prev))
        }
                ...
                >
                    <g ref={groupRef}>
                        ...
                        {svgPaths.map((pathData, pathIndex) => {
                            ...
                            const dashoffset =
                                pathLength > 0 ? (1 - effectiveProgress) * pathLength : 0
